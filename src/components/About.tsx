...
export const About = () => {
  return (
    <section id="about" className="py-16 md:py-24 relative overflow-hidden bg-white/30 dark:bg-transparent backdrop-blur-[1px] transition-colors duration-300">
      <div className="w-full mx-auto px-6 max-w-7xl grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
        ...
        >
          ...
          <div className="absolute -bottom-8 -right-8 glass-panel p-8 rounded-3xl shadow-2xl hidden md:block border border-white/20">
            <div className="text-4xl font-display font-bold text-blue-600 dark:text-blue-500 mb-1 flex items-baseline">
              <AnimatedCounter from={0} to={10} />
              <span className="ml-1">+</span>
            </div>
            <div className="text-sm text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wider">Years of Excellence</div>
          </div>
        </motion.div>
        ...
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <h2 className="text-3xl md:text-5xl font-display font-bold mb-6 text-slate-900 dark:text-white leading-tight">
            Leading the Way in <br className="hidden md:block" />
            <span className="text-blue-600 dark:text-blue-500">Industrial Innovation</span>
          </h2>
          ...
          <Link href="/about" className="px-8 py-4 bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 border border-slate-200 dark:border-white/10 text-slate-900 dark:text-white font-bold rounded-xl transition-all inline-block">
            Learn Our Story
          </Link>
        </motion.div>
      </div>
